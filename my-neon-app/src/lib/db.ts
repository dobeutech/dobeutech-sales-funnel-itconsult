...
export async function getProspectById(id: string): Promise<Prospect | null> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to fetch prospect ${id}: ${response.statusText}`);
  }
  return response.json();
}

export async function updateProspectStatus(id: string, status: string): Promise<void> {
  const response = await fetch(`/api/prospects/${encodeURIComponent(id)}/status`, {
    method: 'PUT',
...
export async function getProspectStats() {
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch prospect stats: ${response.statusText}`);
  }
  return response.json();
}
