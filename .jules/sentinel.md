...
## 2025-05-07 - Remove Direct DB Access from Vite Frontend
**Vulnerability:** The Vite frontend (`my-neon-app`) was accessing the database directly via `@neondatabase/serverless` and exposed credentials in `VITE_DATABASE_URL`, allowing arbitrary query execution by any client.
**Learning:** Vite bundles all `VITE_` prefixed variables into the client app. Any API keys or DB connection strings exposed this way are fully visible to end-users and malicious actors.
**Prevention:** Always refactor database logic out of client-side code and into a secure backend API layer (like Next.js API Routes). Ensure that sensitive credentials are only used server-side and never prefixed with `VITE_` or `NEXT_PUBLIC_`.
